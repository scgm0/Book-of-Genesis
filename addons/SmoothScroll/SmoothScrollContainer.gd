...
func _gui_input(event: InputEvent) -> void:
	...
	if event is InputEventScreenDrag or event is InputEventMouseMotion:
		if content_dragging:
			is_scrolling = true
			if should_scroll_horizontal():
				drag_temp_data[0] += event.position.x - old_drag_pos.x
			if should_scroll_vertical():
				drag_temp_data[1] += event.position.y - old_drag_pos.y
			old_drag_pos = event.position
