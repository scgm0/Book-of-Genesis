...
#pragma warning disable 0219
    ...
    ${ENDIF()}
        ...
        ${data.BlittableCopy ? 'unsafe ' : ''}public static IntPtr Constructor(IntPtr isolate, IntPtr info, int paramLen, long data)
        {
            try
            {
${IF(data.Constructor, () => $`
...
${FOR(toJsArray(data.Methods).filter(item => !item.IsLazyMember), method => $`
        ...
        ${data.BlittableCopy && !method.IsStatic ? 'unsafe ' : ''}public static void ${(method.IsStatic ? "F" : "M")}_${method.Name}(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
    ...
    ${FOR(toJsArray(data.Properties).filter(property => !property.IsLazyMember), property => {
            if (property.HasGetter) {
                $`
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        ${data.BlittableCopy && !property.IsStatic ? 'unsafe ' : ''}public static void G_${property.Name}(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
            ...
            if (property.HasSetter) {
                ...
                $`
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        ${data.BlittableCopy && !property.IsStatic ? 'unsafe ' : ''}public static void S_${property.Name}(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                ${!property.IsStatic ? `var obj = ${getSelf(data)};` : ''}
    ...
    ${IF(data.GetIndexs.Length > 0, () => {
            ...
            $`
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        ${data.BlittableCopy ? 'unsafe ' : ''}public static void GetItem(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
    ...
    ${IF(data.SetIndexs.Length > 0, () => {
            ...
            $`
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        ${data.BlittableCopy ? 'unsafe ' : ''}public static void SetItem(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                var obj = ${getSelf(data)};
    ...
    ${FOR(toJsArray(data.Operators).filter(oper => !oper.IsLazyMember), operator => $`
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        public static void O_${operator.Name}(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
    ...
    ${FOR(toJsArray(data.Events).filter(ev => !ev.IsLazyMember), eventInfo => {
            if (eventInfo.HasAdd) {
                ...
                $`
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        ${data.BlittableCopy && !eventInfo.IsStatic ? 'unsafe ' : ''}public static void A_${eventInfo.Name}(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
            ...
            if (eventInfo.HasRemove) {
                ...
                $`
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        ${data.BlittableCopy && !eventInfo.IsStatic ? 'unsafe' : ''}public static void R_${eventInfo.Name}(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                ${!eventInfo.IsStatic ? `var obj = ${getSelf(data)};` : ''}
    ...
    ${IF(data.BlittableCopy, () => {
            $`
        unsafe public static ${data.Name} StaticGetter(int jsEnvIdx, IntPtr isolate, Puerts.IGetValueFromJs getValueApi, IntPtr value, bool isByRef)
        {
            ${data.Name}* result = (${data.Name}*)getValueApi.GetNativeObject(isolate, value, isByRef);
            return result == null ? default(${data.Name}) : *result;
        }

        unsafe public static void StaticSetter(int jsEnvIdx, IntPtr isolate, Puerts.ISetValueToJs setValueApi, IntPtr value, ${data.Name} val)
        {
            HeapValue = val;
            fixed (${data.Name}* result = &HeapValue)
            {
